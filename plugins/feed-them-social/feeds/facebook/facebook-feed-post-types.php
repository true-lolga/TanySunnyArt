...
">'. __('Get Directions', 'feed-them-social').'</a>';
			...
			case 'link':
				...
				if ($host == 'www.facebook.com' and $first_dir == 'events') {
					...
					// custom one day ago check
					if ($CustomDateCheck == 'one-day-ago') {
						$FB_event_start_time = date_i18n('l, F jS, Y \a\t g:ia', strtotime($event_data->start_time));
					}
					else {
					$FB_event_start_time = date_i18n($CustomDateFormat, strtotime($event_data->start_time));
					}
